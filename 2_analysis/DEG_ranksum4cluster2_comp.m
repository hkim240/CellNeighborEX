function [pvalue_total,fdr_total,logRatio_total, ci_total, tvalue_total ]=DEG_ranksum4cluster(center_celltype,clusterSize,neiCombUnique,clusterSelect,matchComb,log_data,log_data_artificialDoublets)
...
for i=1:size(log_data,1)
    h_var = vartest2(log_data_artificialDoublets(i,:),log_data(i,find(cellSelect1)));   
    if h_var == 0 || h_var == 1
        if h_var == 1 
            % Welch-t-test: uneqaul variance
            [h,p,ci,stats]=ttest2(log_data_artificialDoublets(i,:),log_data(i,find(cellSelect1)),'Vartype','unequal');
        elseif h_var == 0
            % Student's t-test: equal variance
            [h,p,ci,stats]=ttest2(log_data_artificialDoublets(i,:),log_data(i,find(cellSelect1)));
        end

        pvalue(i)=p;
        logRatio(i)=mean(log_data(i,cellSelect1)+1)-mean(log_data_artificialDoublets(i,:)+1);
        ci_lower=ci(1);
        ci_upper=ci(2);
