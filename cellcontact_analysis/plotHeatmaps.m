...
if size(cellContact_DEGs_IDX,1) > 0
    ...
    for x=1:size(logRatio1_cellContact,1)

        val = logRatio1_cellContact(x)*logRatio2_cellContact(x);
        logRatioMultiplied = [logRatioMultiplied, val];

    end
    
    [~,sortIndex]=sort(logRatioMultiplied,'descend'); % sorted by log-ratio
    cellContact_DEGs_IDX=cellContact_DEGs_IDX(sortIndex);
    ...
    title({neiCombUnique(criterionClusterIndex);['(n=',num2str(length(find(cellSelect1))),')']},'FontSize',14); 
    ...
    colormap(redblue(256));
    set(gca, 'FontName', 'Arial','DefaultAxesTitleFontWeight','normal')
    title({neiCombUnique(clusterSelect(2));['(n=',num2str(length(find(cellSelect2))),')']},'FontSize',14);
    ...
    colormap(redblue(256));
    set(gca, 'FontName', 'Arial','DefaultAxesTitleFontWeight','normal')
    title({neiCombUnique(clusterSelect(3));['(n=',num2str(length(find(cellSelect3))),')']},'FontSize',14);
