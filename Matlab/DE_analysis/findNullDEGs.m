function [null_DEGs, pvalue_null,fdr_null,logRatio_null]=findNullDEGs(center_celltype,clusterSelect,matchComb,neiCombUnique,log_data,log_data_artificialHeteroSpots,gene_name,pCutoff,pCutoff2,lrCutoff,direction)
...
if direction==1 % up-regulated genes 
    ...
    DEGindex(:,criterionClusterIndex)= fdr_total{criterionClusterIndex}<pCutoff2 & logRatio_total{criterionClusterIndex}>lrCutoff;
    geneIndexTemp=find(DEGindex(:,criterionClusterIndex) & sum(DEGindex(:,criterionClusterIndex),2)==1);
    % pvalue_total{criterionClusterIndex}<pCutoff &
